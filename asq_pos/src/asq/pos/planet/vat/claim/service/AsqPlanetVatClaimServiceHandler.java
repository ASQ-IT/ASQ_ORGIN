...
public class AsqPlanetVatClaimServiceHandler extends AbstractJaxRsHandler<AsqPlanetVatClaimServiceRequest, IServiceResponse>{
		...
		try {
			try {
				if(argServiceType.getServiceHandlerId().equalsIgnoreCase("PLANET_AUTH_TOKEN_SRV")) {
					...
					Builder requestBuilder = this.getBaseWebTarget().request(MediaType.APPLICATION_JSON)
							.header("Content-Type", "application/json")
							.header(AsqZatcaIntegrationConstants.Authorization, "Bearer "+argServiceRequest.getToken())			
							.header("Accept-Version", "V2");
	...
	private IServiceResponse  handleResponse(Response argRawResponse) {
		...
		try {  
			return   (IServiceResponse) asqZatcaHelper.convertJSONToPojo(argRawResponse.readEntity(String.class),AsqPlanetVatClaimServiceResponse.class);
		}catch(Exception e) {
			e.printStackTrace();
			return null;
		}
...
//				return (IServiceResponse)asqPlanetVatClaimServiceResponse;
//			}catch(Exception e) {
//				e.printStackTrace();
//				return null;
//			}
//		}
	...
	private String getPlanetAuthToken() throws JsonMappingException, JsonProcessingException {
		...
		Builder requestBuilder = this.getBaseWebTarget().request(MediaType.APPLICATION_JSON)
				...
				.header("Accept-Version", "V2");
		rawResponse = requestBuilder.post(Entity.entity(form, MediaType.APPLICATION_FORM_URLENCODED_TYPE));
		AsqPlanetVatClaimServiceResponse asqPlanetVatClaimServiceResponse = asqZatcaHelper.convertJSONToPojo(rawResponse.readEntity(String.class),AsqPlanetVatClaimServiceResponse.class);
