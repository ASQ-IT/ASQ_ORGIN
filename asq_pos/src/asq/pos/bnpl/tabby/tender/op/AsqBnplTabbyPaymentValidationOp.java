...
public class AsqBnplTabbyPaymentValidationOp extends Operation {
	...
	private IOpResponse validateNotificationServiceResponse(AsqSubmitBnplTabbyServiceResponse response, IRetailTransaction trans) {
		if (null != response && null != response.getError()) {
			...
			if (getScopedValue(AsqValueKeys.ASQ_TAMARA_CUSTOMER_PAYMENT_CONFIRMATION) != null) {
				return requestPreparerRetrievePayment(response, trans);
			} else {
				try {
					Thread.sleep(10000);
	...
	private IOpResponse requestPreparerRetrievePayment(AsqSubmitBnplTabbyServiceResponse response, IRetailTransaction trans) {
		...
		if (response.getError() == null) {
			...
			asqBnplTabbyOrderDetailsObj.setReference_id(Long.toString(trans.getRetailLocationId()).concat("-").concat(Long.toString(trans.getTransactionSequence())).concat("-").concat(timeString));
